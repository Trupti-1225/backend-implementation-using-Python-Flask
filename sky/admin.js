...
let currentEditingOpportunityId = null;
...
function openOpportunityDetails(title, details) {
    ...
    document.getElementById('opportunityDetailApplicants').textContent = details.maxApplicants ? details.maxApplicants : 'Not specified';
...
function resetOpportunityForm() {
    ...
    document.getElementById('opportunityFormTitle').textContent = 'Add New Opportunity';
    document.getElementById('opportunitySubmitButton').textContent = 'Create Opportunity';
}

function startEditingOpportunity(opportunity) {
    openOpportunityModal(opportunity);
}

function openOpportunityModal(opportunity = null) {
    if (opportunity) {
        currentEditingOpportunityId = opportunity.id;
        ...
        document.getElementById('opportunityFormTitle').textContent = 'Edit Opportunity';
        document.getElementById('opportunitySubmitButton').textContent = 'Save Changes';
    } else {
...
function ensureOpportunitySectionActive() {
    const ensureActive = () => {
        // Hide all sections
        document.querySelectorAll('.dash-section').forEach(s => s.classList.remove('active'));
        
        // Show opportunity section
        ...
        // Update nav items
        document.querySelectorAll('.nav-item').forEach(i => i.classList.remove('active'));
        const opportunityNav = document.querySelector('.nav-item[data-page="opportunity"]');
        if (opportunityNav) {
            opportunityNav.classList.add('active');
        }
...
function deleteOpportunity(opportunityId) {
    ...
    fetch(`http://127.0.0.1:5000/delete-opportunity/${opportunityId}`, {
        method: 'DELETE',
        credentials: 'include'
    })
    .then(res => res.json())
    .then(data => {
        if (data.success) {
            ...
            loadOpportunities();
            ensureOpportunitySectionActive();
        } else {
            showToast(data.message || 'Unable to delete opportunity');
        }
    })
    .catch(() => {
        showToast('Unable to delete opportunity. Please try again later.');
    });
}
...
// Handle opportunity form submission
        document.getElementById('opportunityForm').addEventListener('submit', function(e) {
            ...
            const isEditing = currentEditingOpportunityId !== null && currentEditingOpportunityId !== '';
            const url = isEditing
                ? `http://127.0.0.1:5000/update-opportunity/${currentEditingOpportunityId}`
                : 'http://127.0.0.1:5000/create-opportunity';
            ...
            .then(data => {
                if (data.success) {
                    ...
                    showToast(data.message || (isEditing ? 'Unable to update opportunity' : 'Unable to create opportunity'));
                }
            })
            .catch(() => {
                showToast((isEditing ? 'Unable to update opportunity.' : 'Unable to create opportunity.') + ' Please try again later.');
            });
        });
...
document.getElementById('loginForm').addEventListener('submit', function(e) {
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            showError('loginPasswordErr', data.message);
        }
    })
    .catch(() => {
...
// load opportunities on dashboard
function loadOpportunities() {
    const container = document.getElementById('opportunitiesContainer');
    if (!container) return Promise.resolve();
    container.classList.add('opportunities-grid');
    container.innerHTML = '';

    return fetch('http://127.0.0.1:5000/get-opportunities', {
        credentials: 'include'
    })
    .then(res => res.json())
    .then(data => {
        if (!data.success || !data.data || data.data.length === 0) {
            container.innerHTML = '<div class="empty-state"><p>No opportunities found. Add a new opportunity to get started.</p></div>';
            return;
        }
            ...
            card.innerHTML = `
                ...
                <div class="opportunity-skills">
                    ...
                    <div class="skills-tags">
                        ${op.skills && op.skills.length ? op.skills.map(skill => `<span class="skill-tag">${escapeHtml(skill)}</span>`).join('') : ''}
                    </div>
                </div>
                <div class="opportunity-footer">
                    <span class="applicants-count">${escapeHtml(op.max_applicants ? op.max_applicants.toString() + ' applicants' : '0 applicants')}</span>
                    <div class="opportunity-actions">
                        <button type="button" class="view-course-btn view-details-btn">View Details</button>
                        <button type="button" class="view-course-btn edit-btn">Edit</button>
                        <button type="button" class="view-course-btn delete-btn">Delete</button>
                    </div>
                </div>
            `;
            ...
            if (viewBtn) {
                viewBtn.addEventListener('click', function() {
                    openOpportunityDetails(op.name, {
                        ...
                        skills: op.skills || [],
                        maxApplicants: op.max_applicants || 0,
    ...
    .catch(() => {
        container.innerHTML = '<div class="empty-state"><p>Unable to load opportunities. Please try again later.</p></div>';
    });
}
...
document.getElementById('signupForm').addEventListener('submit', function(e) {
    ...
    .then(data => {
        if (data.success) {
            ...
            showError('signupEmailErr', data.message);
        }
    })
    .catch(() => {
        showError('signupEmailErr', 'Server error');
    });
});
...
document.getElementById('forgotForm').addEventListener('submit', function(e) {
    ...
    .then(function(data) {
        ...
        // ✅ ALWAYS SAME MESSAGE (SECURITY RULE)
    ...
    .catch(function() {
        showError('forgotEmailErr', 'Server error');
    });
});
